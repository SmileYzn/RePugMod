...
void CVoteMap::Stop()
{
	...
	{
		gTask.Create(PUG_TASK_EXEC, 5.0f, false, gUtil.ServerChangelevel, (char*)gVoteMap.GetItem(Winner));
...
int CVoteMap::RandomMap(bool Change)
{
	...
	if (Change)
	{
		gTask.Create(PUG_TASK_EXEC, 5.0f, false, gUtil.ServerChangelevel, (char*)this->m_Data[Random].c_str());
