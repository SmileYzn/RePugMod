...
void CVoteMap::Stop()
{
	...
	{
		gTask.Create(PUG_TASK_EXEC, 5.0f, false, SERVER_COMMAND, gUtil.VarArgs("changelevel %s", gVoteMap.GetItem(Winner)));
...
int CVoteMap::RandomMap(bool Change)
{
	...
	if (Change)
	{
		gTask.Create(PUG_TASK_EXEC, 5.0f, false, SERVER_COMMAND, gUtil.VarArgs("changelevel %s", this->m_Data[Random].c_str()));
